...
export const api = {
  ...
  leads: (q = '') => request<Lead[]>(`/api/leads${q ? `?q=${encodeURIComponent(q)}` : ''}`),
...
export function connectRealtime(onEvent: (message: { event: string; data: unknown }) => void) {
  if (!token) return () => {};
  const wsBase = API_URL.replace(/^http/, 'ws');
  const socket = new WebSocket(`${wsBase}/realtime?token=${encodeURIComponent(token)}`);
